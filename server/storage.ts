...
export class Storage {
  ...
  private customizeTemplate(dir: string, repo: Repository) {
    ...
    // Prepend config table to README (keep the template's content)
    const readmePath = join(dir, "README.md");
    if (existsSync(readmePath)) {
      const original = readFileSync(readmePath, "utf-8");
      // Replace the first heading line with the user's repo name
      const customized = original
        ...
        .replace(
          ...
          `| Instance Type | ${dbType === "Aurora" && instanceType.includes("micro") ? "db.t4g.medium" : instanceType} |\n` +
          `| AWS Region | ${region} |`
        );
