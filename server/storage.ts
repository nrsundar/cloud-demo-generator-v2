...
export class Storage {
  ...
  private customizeTemplate(dir: string, repo: Repository) {
    ...
    // Prepend invariant banner + config table, append standard footer
    const readmePath = join(dir, "README.md");
    if (existsSync(readmePath)) {
      const original = readFileSync(readmePath, "utf-8");
      const now = new Date().toISOString().split("T")[0];
      const effectiveInstance = dbType === "Aurora" && instanceType.includes("micro") ? "db.t4g.medium" : instanceType;
      ...
      const configTable =
        `\n| Setting | Value |\n|---------|-------|\n` +
        `| Database | ${dbType} PostgreSQL ${pgVersion} |\n` +
      ...
      const footer =
        ...
        `## Related\n\n` +
        `- [DemoForge](https://github.com/nrsundar/cloud-demo-generator-v2) — generate more demos like this one\n` +
        `- [Aurora PostgreSQL Extensions](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraPostgreSQLReleaseNotes/AuroraPostgreSQL.Extensions.html) — full extension list\n`;

      // Strip old heading, banner, and footer sections
      let body = original
        .replace(/^#\s+.*\n/, "")
        .replace(/^>.*\n/gm, "")
        .replace(/\|.*\|.*\|\n/g, "")
